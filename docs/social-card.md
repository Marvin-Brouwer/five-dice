...
It is the masthead, stacked: the cardboard page, a sheet of paper with a mesh
behind it, the five dice tossed above the wordmark on their black band, and the
tagline written on the paper below the rule.
...
card is real CSS (the tokens, the paper mesh, the mono wordmark, the hand the
tagline is written in) and a browser is the only thing that renders that
faithfully. Passing `--html <path>` writes the page out to open yourself.
...
The fonts are fetched from Google Fonts — the same faces `index.html` loads —
and inlined into the page before the shot, so this is the one script here that
wants a network connection. Inlining them is also what makes the shot
...
and that the tags describing it still match the image they describe.
