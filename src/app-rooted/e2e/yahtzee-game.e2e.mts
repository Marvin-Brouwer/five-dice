...
/**
 * Thirteen yahtzees, every one of them committed to the flush row.
 *
...
function yahtzeeFor(round: number): DiceTuple {
...
test('thirteen yahtzees stacked onto the flush row', async ({ page }) => {
	...
	for (let round = 1; round <= rounds; round++) {
		...
		await game.enterRoll(yahtzeeFor(round), 'flush', sacrifice)

		const flush = await game.row('flush')
		expect(flush.score, `flush after ${round} yahtzee(s)`).toBe(String(50 + (round - 1) * 100))
		expect(flush.badge, `badge after ${round} yahtzee(s)`).toBe(round === 1 ? null : `+${round - 1}`)
