...
export class InvalidScoreApplicationError extends Error {
	...
	public static invalidScoreApplication(scorePad: ReadonlyState<ScorePad>, application: ScoreApplication) {
	...
	public static scoreAlreadyApplied(scorePad: ReadonlyState<ScorePad>, application: ScoreApplication) {
		const message = `the attempted score [${application.score}] for field '${application.field}' ` + 
            'was already applied in a previous round'
		return new InvalidScoreApplicationError(message, application, scorePad)
	}
	public static noDiscardOnFlush(scorePad: ReadonlyState<ScorePad>, application: ScoreApplication) {
		const message = 'when applying for multiple \'flush\'s, a discard is required'
		return new InvalidScoreApplicationError(message, application, scorePad)
	}
	public static flushDiscarded(scorePad: ReadonlyState<ScorePad>, application: ScoreApplication) {
		const message = 'you cannot append values to a \'flush\' that is already discarded'
		return new InvalidScoreApplicationError(message, application, scorePad)
	}
	public static cannotDiscardFlush(scorePad: ReadonlyState<ScorePad>, application: ScoreApplication) {
	...
	private constructor (
        ...
        public scorePad: ReadonlyState<ScorePad>
	) {
		super(message)
	}
}