...
function* generateAllPossibleNumbers(): Generator<[string, ValidScore]> {
	...
	while(e <= 6) {
		const generatedScore = score([a,b,c,d,e])
...
type PatternDefinition = (r: [a: DieValue, b: DieValue, c: DieValue, d: DieValue, e: DieValue]) => ValidScore
export function generateRandomScores(...validPatterns: Array<string>): [
	...
	const allowedScoreMap = shufflePatterns(validPatterns)
		.reduce((accumulator, current) => {
			...
			for (const rollAttempt of Array.from(allRolls.values())
				.filter(scoreAttempt => new Set(scoreAttempt).size == 5)
			){
				const score = toPatternGenerator(current)(rollAttempt)
	...
	if (settings.writeOutMappedScores) {
		...
		for(const s of allowedScores) {
			console.debug(`- ${s}`)
		}
		console.debug('disallowedScores')
		for(const s of Array.from(disallowedScores)) {
			console.debug(`- ${s}`)
		}
	}
